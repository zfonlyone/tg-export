# TG Export - Telegram 全功能导出工具 v2.3.2
...
### 🚀 TDL 下载机制重构
- **Detached Polling**: 彻底重构了 TDL 的调用方式，采用分离式轮询机制，完美解决了“下载成功信号无法捕获”以及“读取日志流导致卡死”的问题。

### 🛡️ 默认全量扫描
- **新建任务全量化**: 创建新任务时默认进行全量历史消息扫描，防止因增量逻辑误判导致漏消息。增量扫描仍可在高级选项中手动开启。

### 📚 文档更新
- **流程透明化**: 新增 [TDL 下载流程文档](docs/tdl_download_flow.md)，详细拆解了下载与校验的内部逻辑。

### 📦 配置管理现代化 (v2.3.1)
- **全面弃用 .env**: 配置文件统一迁移至 `config/config.yml`。
